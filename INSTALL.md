...
- 설치 후 새 세션에서 `/humanize-korean`(또는 `/humanize`, `/humanize-quick`, `/humanize-redo`), 혹은 자연어 트리거("이 글 AI 티 없애줘")로 발동.
- 업데이트: `/plugin marketplace update im-not-ai` 후 `/plugin update humanize-korean`.
- 제거: `/plugin uninstall humanize-korean`.
- 구성요소: 스킬 4개(humanize-korean·humanize·humanize-quick·humanize-redo) + 서브에이전트 9개가 함께 설치됩니다.

...
> ⚠️ **장르 한계.** 임계는 **학술 초록에서만 검증**됐다. 블로그·댓글 실측에서는
> 모든 지표가 판별에 실패했다(최대 |0.5차| 0.192). 비형식 산문 입력에서는
> route_hint 를 근거로 여기지 말고, 사용자에게 그 사실을 알린다.
>
...
**세 게이트를 순서대로 Bash 로 실행한다.** 앞의 둘은 '너무 많이 했다'를,
세 번째는 '너무 적게 했다'를 잡는다. 전부 언어 무관으로 검증된 코드다.
...
> **왜 두 번째 게이트가 필요한가**: 실측(2026-09-02)에서 윤문이 목표 지표를 전부
...
- 게이트 결과를 숨기지 않는다 — 변경률·역주입·과소윤문 세 판정을 함께 보고한다.

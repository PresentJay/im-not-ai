...
(OpenAI GPT)에서 14편을 뽑아 같은 인간 코퍼스에 붙였다.

| 지표 | 인간 | Claude | GPT | AUC(Claude) | **AUC(GPT)** |
|---|---|---|---|---|---|
| tricolon | 0.00 | 1.67 | **6.67** | 0.681 | **0.860** |
| comma_segment_length | 9.68 | 8.34 | **6.90** | 0.281 | **0.040** |
| sentence_length_dispersion | 9.86 | 8.26 | **5.35** | 0.349 | **0.055** |
| en1_participial | 0.00 | 0.00 | 1.67 | 0.587 | 0.638 |

**세 신호 전부 GPT 에서 더 강하다.** 라우터 분리도는 Claude 0.65 → **GPT 1.33**
...
n=14, 단일 모델(codex CLI 기본값)이다. Gemini·Llama 는 여전히 미검증.

### 남은 한계

- 승격은 `comma_segment_length` 하나뿐이고, 그것도 sonnet 에서 약하다(0.408).

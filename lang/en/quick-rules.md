...
> 원인이 아니었다.**
>
> ③ **R2**(인간 100편 3출처 · AI 102편 실사용자 프롬프트 · 본문 중간 발췌 ·
> 지표 16종 · CI 판정) — **여기서 신호가 나왔다.**
>
> R2 결론 셋:
...
> 2. **tricolon(3항 등위, blader #10)이 블로그 최강 후보다** — AUC 0.681,
>    인간 중앙값 세 출처 모두 0.00. 다만 기준(0.20)에 0.019 모자라 **승격 보류**.
> 3. 쉼표 계열·EN-2 는 이 장르에서 **모델 개인어**다(모델마다 부호 반대).
> 4. **GPT 산문에서 더 잘 선다** — 같은 신호로 분리도 1.33(Claude 0.65).
>    tricolon AUC 0.860. 블로그 신호는 Claude 개인어가 아니다.
>

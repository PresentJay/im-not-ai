...
> 원인이 아니었다.**
>
> 2회차가 준 진단이 더 구체적이다: **모델 방향이 갈린다.** 초록 셀의 최강 신호였던
> 쉼표 계열·EN-2 가 에세이에서는 모델마다 부호가 반대다(EN-2: opus 0.736 vs
> haiku 0.162). 방향이 일치하는 건 분산·쉼표 절 길이·**EN-1** 셋뿐이고, 그마저
> 크기가 작다. 이 장르에서 그 지표들은 AI 티가 아니라 **모델 개인어**다(G1 미통과).
> **비형식 산문에 쓸 때는 규칙을 근거 있는 것으로 여기지 말 것.**
> 상세: `lang/en/baseline.json` genres.blog / genres.blog_essay.
>

"""humanize-quick 이 참조하는 패턴 ID 가 taxonomy 에서 사라지지 않았는지 검사.

quick-rules.md 는 build_quick_rules.py 가 생성해 ID 드리프트를 구조적으로 막지만,
humanize-quick/SKILL.md 는 실측 판별력 상위 6개만 손으로 고른 목록이라 생성물이
...
_SKILL = _ROOT / "skills" / "humanize-quick" / "SKILL.md"
...
def test_quick_six_exist_in_taxonomy() -> None:
    ...
    assert not missing, f"humanize-quick 이 taxonomy 에 없는 ID 를 가리킨다: {missing}"
